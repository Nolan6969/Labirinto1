const feedbackElement = document.getElementById("feedback"); // Adicionado elemento HTML para mensagens de feedback
let currentTheme = "default"; // Adicionada variável global para o tema

// Função para alterar o tema
function setTheme(theme) {
  currentTheme = theme;
  // Lógica para aplicar o novo tema
  document.body.style.backgroundColor = theme === "dark" ? "#222" : "#fff";
  // Adicione mais personalizações conforme necessário
}

// Adicionado método para animar o movimento da bola
function animateBallMove(x, y) {
  gsap.to("#ball", { duration: 0.5, x: x, y: y, ease: "power2.inOut" });
}

// Adicionado método para exibir feedback
...
function startTimer() {
  timer = setTimeout(() => {
    showFeedback("Game Over - Time Out");
    end(true);
...
const round = [...Array(maxC)].map(
  (_, i) => 2 ** keys.findLastIndex((j) => j < i + 1) * 8
);
...
const walks = {
  ...
  prims: async function (cells, walls, from) {
    ...
    while (eligable.size) {
      ...
      const from = chooseFrom(
        cells.fieldNeighbours(step).filter((n) => n.maze)
      );
  ...
  depthFirst: async function (cells, walls, from) {
    do {
      const step = chooseFrom(
        cells.fieldNeighbours(from).filter((n) => !n.maze)
      );
...
async function init(algorithm) {
  ...
  document
    .querySelector("svg")
    .setAttribute("viewBox", [-size, -size, 2 * size, 2 * size].join(" "));
...
document.getElementById("setDarkTheme").addEventListener("click", setDarkTheme);
document
  .getElementById("setDefaultTheme")
  .addEventListener("click", setDefaultTheme);
// Adicione estas linhas onde você configura o evento para o botão de replay
replay.onpointerup = (e) => {
  level = 1;
  gsap
    .timeline({ onComplete: restartGame })
    .set(".txt2, .txt3, #replay", { autoAlpha: 0 })
    .set("#grid, .txt1", { attr: { fill: "#0f2" } })
    .fromTo(
      ".txt1",
      { scale: 0, opacity: 0, y: 0, textContent: "LEVEL " + level },
